...
# Copy engine modules into old app (idempotent)
for dir in trading opentrade opentrader scripts; do
  if [[ -d "$ENGINE_ROOT/$dir" ]]; then
    echo "→ Sync $dir/"
    mkdir -p "$OLD_APP/$dir"
    rsync -a --delete "$ENGINE_ROOT/$dir/" "$OLD_APP/$dir/"
  fi
done
...
echo ""
echo "This serves YOUR old index.html + all API (/api/candles, /api/symbols,"
echo "/api/bookmap/stream, backtest, journal, optimizer, MT5 auto-sync)."
