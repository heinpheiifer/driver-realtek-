...
cp "$ENGINE_ROOT/scripts/connect_opentrader_chart.sh" "$OLD_APP/scripts/"
cp "$ENGINE_ROOT/scripts/start_opentrader_django.sh" "$OLD_APP/scripts/"
cp "$ENGINE_ROOT/scripts/start_blackbull_bridge.sh" "$OLD_APP/scripts/"
cp "$ENGINE_ROOT/scripts/start_opentrader_stack.sh" "$OLD_APP/scripts/"
...
if ! grep -q "^MT5_WINE_SYNC=" "$ENV_FILE" 2>/dev/null; then
